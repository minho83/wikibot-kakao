...
class BookmarkCreator:
    ...
    def _call_gpt(self, title: str, board_name: str, content: str) -> dict | None:
        """GPT-4o-mini로 책갈피 데이터 추출"""
    ...
    def create_bookmark(self, raw_post: dict) -> dict | None:
        ...
        # GPT 호출
        gpt_result = self._call_gpt(title, board_name, content)
        if not gpt_result:
            return None
        ...
        for filepath in glob.glob(os.path.join(data_path, "*.json")):
            try:
                with open(filepath, "r", encoding="utf-8") as f:
                    data = json.load(f)
                if not data.get("bookmark_created", False):
                    posts.append(data)
            except Exception as e:
